...
OGLTexture::OGLTexture(TexType type) : GTexture(""), type(type)
{
	...
	{
		glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32, 1920, 1080, 0, GL_DEPTH_COMPONENT, GL_FLOAT, nullptr);
	}
}
...
void OGLTexture::resize(uint32_t width, uint32_t height, char* data)
{
	glBindTexture(GL_TEXTURE_2D, texture);
	if (type == COLOR)
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
